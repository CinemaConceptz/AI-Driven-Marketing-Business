...
export default function EpkPage() {
  ...
  const socialLinks = [];
  if (profile?.links?.spotify) socialLinks.push({ platform: "spotify", url: profile.links.spotify, color: "text-green-400" });
  if (profile?.links?.soundcloud) socialLinks.push({ platform: "soundcloud", url: profile.links.soundcloud, color: "text-orange-400" });
  if (profile?.links?.bandcamp) socialLinks.push({ platform: "bandcamp", url: profile.links.bandcamp, color: "text-teal-400" });
  if (profile?.links?.appleMusic) socialLinks.push({ platform: "appleMusic", url: profile.links.appleMusic, color: "text-pink-400" });
  if (profile?.links?.youtube) socialLinks.push({ platform: "youtube", url: profile.links.youtube, color: "text-red-400" });
  if (profile?.links?.instagram) socialLinks.push({ platform: "instagram", url: profile.links.instagram, color: "text-purple-400" });
  if (profile?.links?.website) socialLinks.push({ platform: "website", url: profile.links.website, color: "text-blue-400" });
      ...
      {hasEnhancedEpk && epk && (
        <div className="space-y-6">
          ...
          <div className="glass-panel rounded-3xl px-8 py-8 text-center">
            <h3 className="text-xs uppercase tracking-widest text-slate-500 mb-4">Contact</h3>
