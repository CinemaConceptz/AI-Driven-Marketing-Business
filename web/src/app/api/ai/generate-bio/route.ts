...
export async function POST(req: Request) {
  try {
    ...
    if (!artistName) {
      return NextResponse.json({ ok: false, error: "Artist name required" }, { status: 400 });
    }

    const genreList = genres?.length > 0 ? genres.join(", ") : "electronic music";
    const hasSpotify = !!links?.spotify;
    const hasSoundcloud = !!links?.soundcloud;

    // Build context for AI
    const platformContext = hasSpotify || hasSoundcloud 
